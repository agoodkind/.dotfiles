...
#
# Note: This module uses MAGENTA and GRAY from colors.bash.
# Always source colors.bash before sourcing this file.
...
if [[ -f "$HOME/.cache/dotfiles_debug_enabled" ]] || [[ "${DEBUG:-false}" == "true" ]]; then
    printf "[DEBUG] defaults.bash: checking arguments: %s\n" "$*"
fi
for arg in "$@"; do
    case $arg in
        --use-defaults | -d)
...
if [[ "$found_use_defaults" == "true" ]]; then
    ...
    export USE_DEFAULTS
    if [[ -f "$HOME/.cache/dotfiles_debug_enabled" ]] || [[ "${DEBUG:-false}" == "true" ]]; then
        printf "[DEBUG] defaults.bash: found --use-defaults flag, setting USE_DEFAULTS=true\n"
    fi
elif [[ -z "${USE_DEFAULTS:-}" ]]; then
    USE_DEFAULTS=false
    export USE_DEFAULTS
    if [[ -f "$HOME/.cache/dotfiles_debug_enabled" ]] || [[ "${DEBUG:-false}" == "true" ]]; then
        printf "[DEBUG] defaults.bash: no --use-defaults flag found, setting USE_DEFAULTS=false\n"
    fi
fi
