...
5. **GitHub issues** — when report §7 says possible or likely, or your
   ...
   when your partner explicitly requests that external write. `gh` cannot attach files; if a bundle exists, give
...
6. **Export** — only when your partner asks for a bundle; never build one
   ...
   Archive (`zip -r` or `tar -czf`) only when your partner explicitly requests the archive after reviewing the file list. With the archive
...
- **No superpowers diagnosis.** Report §7 states involvement and stops.
  ...
  partner either.
- **External-write authorization.** Create an archive, issue, or comment only
  when your partner explicitly requests that exact action after seeing its contents.
