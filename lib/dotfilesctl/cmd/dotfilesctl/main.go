...
func run(args []string) int {
	...
	if logPath == "" {
		logPath = filepath.Join(os.Getenv("HOME"), ".cache", "dotfiles", "dotfilesctl.log")
	}
...
func runSync(args []string) int {
	for _, arg := range args {
		if arg == "-h" || arg == "--help" {
			logInfo("Usage:")
			logInfo("  dotfilesctl sync [--repair] [--quick] [--skip-git] [--skip-network] [--skip-cursor-sync] [--dry-run] [--use-defaults]")
			return 0
		}
	}
...
func runDispatch(args []string) int {
	if len(args) > 0 {
		for _, arg := range args {
			if arg == "-h" || arg == "--help" {
				logInfo("Usage: dotfilesctl dispatch")
				return 0
			}
		}
	}
...
func runCursorSync(args []string) int {
	if len(args) > 0 {
		for _, arg := range args {
			if arg == "-h" || arg == "--help" {
				logInfo("Usage: dotfilesctl cursor-sync")
				return 0
			}
		}
	}
...
func printUsage() {
	logInfo("Usage:")
	logInfo("  dotfilesctl sync [--repair] [--quick] [--skip-git] [--skip-network] [--skip-cursor-sync] [--dry-run] [--use-defaults]")
	logInfo("  dotfilesctl dispatch")
	logInfo("  dotfilesctl sync-claude-repo [path]")
	logInfo("  dotfilesctl cursor-sync")
	logInfo("  dotfilesctl install [--use-defaults] [--quick] [--skip-git] [--skip-network] [--repair]")
	logInfo("  dotfilesctl uninstall [--purge-packages]")
	logInfo("  dotfilesctl version")
}
